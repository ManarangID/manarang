        ...
        <div class="row mb-2">
          ...
          <div class="col-sm-6">
            <ol class="breadcrumb float-sm-right">
              ...
              <li class="breadcrumb-item active">{{ __('pages.edit_title') }}</li>
            </ol>
          </div>
        </div>
      </div><!-- /.container-fluid -->
          ...
          <div class="col-12">
            <div class="card">
              <div class="card-header">
                <h3 class="card-title">{{ __('pages.edit_title') }}</h3>
              ...
              <!-- /.card-header -->
            	<!-- form start -->
            	<form id="createForm" method="POST" enctype="multipart/form-data" action="{{ route('pages.update', Hashids::encode($pages->id)) }}">
                	@method('PUT')
					@csrf
            	  <div class="card-body">
            		<div class="row">
            		  ...
            		  <div class="col-md-6">
            		    <div class="form-group">
            		      ...
            		      <input type="text" class="form-control" id="seotitle" name="seotitle" value="{{ $pages->seotitle }}">
            		    </div>
            		    <!-- /.form-group -->
            		  </div>
            		  <!-- /.col -->
            		...
            		<div class="row">
            		  <div class="col-md-12">
            		    <div class="form-group">
            		      ...
            		      <textarea class="p-3" id="content" name="content">{{ $pages->content }}</textarea>
            		    </div>
            		    <!-- /.form-group -->
            		  </div>
            		  <!-- /.col -->
            		  <div class="col-md-12">
						<div class="form-group">
							...
							<div class="custom-file" x-data="showImage()">
								...
								<label class="custom-file-label" for="customFile">@if ( $pages->picture == '') Choose file @else {{ $pages->picture }} @endif</label>
							</div>
            		    </div>
            		    <!-- /.form-group -->
						<div class="form-group">
							<label>{{ __('pages.active') }}</label>
							<select id="active" name="active" class="form-control select2bs4" style="width: 100%;">
								<option value="Y" {{ $pages->active == 'Y' ? 'selected' : '' }}>{{ __('pages.active') }}</option>
								<option value="N" {{ $pages->active == 'N' ? 'selected' : '' }}>{{ __('post.draft') }}</option>
							</select>
						</div>
						<!-- /.form-group -->
            		  </div>
