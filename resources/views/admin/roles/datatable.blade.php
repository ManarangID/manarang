        ...
        <div class="row mb-2">
          ...
          <div class="col-sm-6">
            <ol class="breadcrumb float-sm-right">
              ...
              <li class="breadcrumb-item"><a href="{{ route('roles.index') }}">{{ __('general.roles') }}</a></li>
            ...
            <div class="card">
              <div class="card-header">
				...
				<div class="card-tools">
					<button onclick="location.href='{{ route('roles.create') }}'" type="button" class="btn btn-primary"> <i class="fas fa-plus"></i> {{ __('general.add') }}</button>
				</div>
              </div>
              <!-- /.card-header -->
              <div class="card-body">
                <table id="example1" class="table table-bordered table-striped">
                  ...
                  @foreach ($roles as $key => $role)
                    <tr id="tr_{{ $role->id }}">
                      ...
                      <td>
                          <form method="POST" action="{{ route('roles.destroy', Hashids::encode($role->id)) }}">
                            @csrf
                            <input name="_method" type="hidden" value="DELETE">
                            <a href="{{ route('roles.edit', Hashids::encode($role->id)) }}" class="btn btn-sm btn-info"><i class="fas fa-pen"></i></a>
                  ...
                  <tfoot>
                    <tr>
                      ...
                      <td colspan="4">
                      <a href="" class="btn btn-sm btn-danger removeAll" id="removeAll"><i class="fa fa-trash"></i> {{ __('general.delete_selected') }}</a>
                      </td>
                    </tr>
                  </tfoot>
                </table>
        ...
        $('.removeAll').on('click', function(e) {
            ...
            if (studentIdArr.length <= 0) {
                ...
                if (confirm("Are you sure?")) {
                    ...
                    $.ajax({
                        url: "{{url('dashboard/deleteallroles')}}",
                        type: 'DELETE',
                        method: 'POST',
                        headers: {
